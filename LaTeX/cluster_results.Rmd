...
# all_event_daily_anom_0.5$season[is.na(all_event_daily_anom_0.5$season)] <- "event"
# # Save
...
all_event_daily_anom_0.5$season <- factor(all_event_daily_anom_0.5$season, levels = c("summer", "autumn", "winter", "spring", "event"))

# Vector of values for plotting
label_colour <-  c("royalblue3", "springgreen4") # This shows data type
label_font <- c("daily", "event")
label_shape <- c(15, 16, 17, 18, 8) # This shows seasons
...
point.lab <- function(n) {
    if (is.leaf(n)) {
        ...
        attr(n, "nodePar") <- c(a$nodePar, list(lab.col = lab_col, col = lab_col,
                                                pch = lab_shape, cex = 0.5, lab.cex = 0.3)) 
...
legend("right", legend = c("summer", "autumn", "winter", "spring", "event"),
