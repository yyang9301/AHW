...
for (i in 1:length(vars)){ # the variable level
  var1 <- vars[i]
  for (j in 1994:2016){ # the year level
    for (k in month.special){ # the month level
      nc.file <- paste0("http://dapds00.nci.org.au/thredds/dodsC/gb6/BRAN/BRAN_2016/OFAM/",var1,"_",j,"_",k,".nc")
      # ncfilename <- paste0(BRANdir,basename(nc.file)) # rather save the output in .Rdata format
      ncfilename <- paste0(BRANdir,var1,"_",j,"_",k,".Rdata")
      if (!exists(ncfilename)){ # check if file has not already been saved
        cat("Now downloading", basename(nc.file),"\n")
